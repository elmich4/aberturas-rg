...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  const { vendedor, loading, logout } = useAuth()
      ...
      <aside style={{
        ...
        <nav style={{ flex: 1, padding: '12px 8px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          {MENU.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} style={{
                ...
                textDecoration: 'none',
                background: active ? 'rgba(214,40,40,0.15)' : 'transparent',
          ...
          <div style={{ borderTop: '1px solid #1e1e1e', paddingTop: 12, display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Link href="/calculadoras" target="_blank" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', textDecoration: 'none', color: '#555', fontSize: 12 }}>
              🧮 Calculadoras →
            </Link>
          </div>
        </nav>
        ...
        <div style={{ padding: '12px 16px', borderTop: '1px solid #1e1e1e' }}>
          <div style={{ fontSize: 12, color: '#888', marginBottom: 6 }}>👤 {vendedor.nombre}</div>
